...
// ── MASTER PASSWORD + CUSTOM SLOT PASSWORDS ────────────────────────
var MASTER_PW_KEY = "pl_master_pw";
var CUSTOM_PWS_KEY = "pl_custom_pws";
function getMasterPw(){ try{ return localStorage.getItem(MASTER_PW_KEY)||""; }catch(e){ return ""; } }
...
function setCustomPw(uid,pw){ try{ var c=getCustomPws(); c[uid]=pw; localStorage.setItem(CUSTOM_PWS_KEY,JSON.stringify(c)); }catch(e){} }
...
var lastMovedLat = null;
var lastMovedLng = null;
var lastMovedTime = Date.now();
var stallAlertSent = false;

function checkGPSStall(lat,lng,driverId,driverName,jobId){
  var moved = !lastMovedLat || Math.abs(lat-lastMovedLat)>0.0001 || Math.abs(lng-lastMovedLng)>0.0001;
  if(moved){ lastMovedLat=lat; lastMovedLng=lng; lastMovedTime=Date.now(); stallAlertSent=false; return; }
  var stallMs=(Date.now()-lastMovedTime);
  var cfg=loadSafetyConfig();
  var stallLimit=(cfg.stallMinutes||GPS_STALL_MINUTES)*60000;
  if(stallMs>stallLimit&&!stallAlertSent&&jobId){
    stallAlertSent=true;
    sendEmergencyAlert(driverId,driverName,lat,lng,jobId,"GPS STALL — truck has not moved for "+(Math.round(stallMs/60000))+" minutes during active job");
  }
}
...
// ═══════════════════════════════════════════════════════════════════
var SAFETY_KEY = "potent_safety_config";
var DEFAULT_PANIC_PHRASE = "eagle one";
var GPS_STALL_MINUTES = 10; // alert if truck doesn't move for this long during active job
var GPS_STALL_CHECK_MS = 60000; // check every 60 seconds

function loadSafetyConfig(){
  try{ return JSON.parse(localStorage.getItem(SAFETY_KEY)||'{"panicPhrase":"eagle one","monitoredDrivers":{},"alertPhone":"7706484228"}'); }
  catch(e){ return {panicPhrase:"eagle one",monitoredDrivers:{},alertPhone:"7706484228"}; }
}
function saveSafetyConfig(c){ try{ localStorage.setItem(SAFETY_KEY,JSON.stringify(c)); }catch(e){} }

async function sendEmergencyAlert(driverId,driverName,lat,lng,jobId,reason){
  var msg="POTENT OS ALERT: "+reason+" | Driver: "+driverName+" | Job: "+(jobId||"none")+" | Location: https://maps.google.com/?q="+lat+","+lng+" | Time: "+new Date().toLocaleTimeString();
  // Log to Supabase
  try{
...
// OFFLINE SUPPORT
var OFFLINE_QUEUE_KEY = "potent_offline_queue";

// ── SUPABASE CRUD HELPERS ─────────────────────────────────────────
function sbGet(table,qs){return fetch(SUPABASE_URL+"/rest/v1/"+table+(qs?"?"+qs:""),{headers:{apikey:SUPABASE_ANON_KEY,Authorization:"Bearer "+SUPABASE_ANON_KEY}}).then(function(r){return r.json();}).catch(function(){return [];});}
function sbUpsert(table,data){return fetch(SUPABASE_URL+"/rest/v1/"+table,{method:"POST",headers:{apikey:SUPABASE_ANON_KEY,Authorization:"Bearer "+SUPABASE_ANON_KEY,"Content-Type":"application/json",Prefer:"resolution=merge-duplicates,return=representation"},body:JSON.stringify(Array.isArray(data)?data:[data])}).then(function(r){return r.json();}).catch(function(){return null;});}
function sbDelete2(table,col,val){return fetch(SUPABASE_URL+"/rest/v1/"+table+"?"+col+"=eq."+encodeURIComponent(val),{method:"DELETE",headers:{apikey:SUPABASE_ANON_KEY,Authorization:"Bearer "+SUPABASE_ANON_KEY}}).catch(function(){});}
...
function addToOfflineQueue(job){
  var q=getOfflineQueue();
  if(q.find(function(j){return j.id===job.id;})) return;
  q.push(Object.assign({},job,{_offlineId:"OFF-"+Date.now(),_offlinePending:true}));
  saveOfflineQueue(q);
}
function removeFromOfflineQueue(jobId){
  saveOfflineQueue(getOfflineQueue().filter(function(j){return j.id!==jobId;}));
}
...
// ── JOB DOCUMENTS — BOL, RATE CON, POD, etc. ─────────────────────
var JOB_DOCS_KEY = "pl_job_docs";
function loadJobDocs(jobId){ try{ var a=JSON.parse(localStorage.getItem(JOB_DOCS_KEY)||"{}"); return Array.isArray(a[jobId])?a[jobId]:[]; }catch(e){ return []; } }
function saveJobDocs(jobId,docs){ try{ var a=JSON.parse(localStorage.getItem(JOB_DOCS_KEY)||"{}"); a[jobId]=docs; localStorage.setItem(JOB_DOCS_KEY,JSON.stringify(a)); }catch(e){} }

function JobDocumentsPanel(props){
...
function DriverDocViewer(props){
    var jobId=props.jobId;
    var [docs]=useState(function(){return loadJobDocs(jobId);});
    if(!docs||docs.length===0)return null;
    return React.createElement("div",{style:{marginTop:10,padding:"10px 0"}},
        React.createElement("div",{style:{fontSize:10,color:C.orange,fontWeight:700,textTransform:"uppercase",letterSpacing:1,marginBottom:8}},"📎 "+docs.length+" Document"+(docs.length!==1?"s":"")+" From Dispatch"),
        docs.map(function(d){
...
function CallBtns(props) {
    ...
    var gvUrl = "https://voice.google.com/calls?a=nc,+" + (n.length===10?"1"+n:n);
    return React.createElement("div", { style: { display: "flex", gap: 6, flexWrap:"wrap" } },
        ...
        React.createElement("a", { href: gvUrl, target:"_blank", rel:"noopener", style: { textDecoration: "none" } },
            React.createElement(Btn, { variant: "muted", style: { padding: "6px 12px", fontSize: 11, background:"#1a7a3c22", color:"#1DB954", borderColor:"#1DB95444" } }, "\uD83D\uDCF2 GVoice")),
    ...
    return React.createElement("div", { style: { minHeight: "100vh", background: C2.black, fontFamily: ff, color: C2.white } },
        ...
        React.createElement("div", { style: { background: "linear-gradient(180deg,#0d0d0d,#080808)", padding: "70px 20px 50px", textAlign: "center", borderBottom: "1px solid " + C2.border } },
            ...
            React.createElement("div", { style: { fontSize: 14, color: C2.orange, fontWeight: 700, marginBottom: 30 } },
                totalFeatures,
                "+ features across 20 categories \u00B7 One-time license \u00B7 Own it forever"),
            ...
            React.createElement("div", { style: { maxWidth: 720, margin: "0 auto" } },
                ...
                React.createElement("div", { style: { display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 } },
                    ...
                    React.createElement("div", { style: { background: C2.green + "10", border: "1px solid " + C2.green + "33", borderRadius: 12, padding: "20px" } },
                        ...
                        React.createElement("div", { style: { marginTop: 12, textAlign: "center", fontSize: 20, fontWeight: 900, color: C2.green } }, "License pays for itself in 16 months on GPS alone. 5-year savings: ~$2.2M"))))),
        ...
        React.createElement("div", { style: { padding: "60px 20px", borderBottom: "1px solid " + C2.border } },
            React.createElement("div", { style: { maxWidth: 900, margin: "0 auto" } },
                React.createElement("div", { style: { textAlign: "center", marginBottom: 36 } },
                    React.createElement("div", { style: { fontSize: 11, color: C2.orange, fontWeight: 700, letterSpacing: 2.5, textTransform: "uppercase", marginBottom: 8 } }, "No One Else Has This"),
                    React.createElement("div", { style: { fontSize: 28, fontWeight: 900, color: C2.white, marginBottom: 14 } }, "5 features no other platform offers."),
                    React.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 10, marginBottom: 36 } },
                        [["\uD83D\uDCF4 Offline PWA — Works Without Internet", "Navy-grade offline mode. Installs like a real app on any iPhone or Android. No app store. Works in dead zones, warehouses, rural areas. No competitor has this.", "$10k+/yr saved"],
                         ["\uD83C\uDFA4 Voice-Activated Quoting", "Hands-free dispatch while on the phone. Say the customer name, pickup, service type and fields fill automatically. Saves 15+ hrs/week of dispatcher time.", "15+ hrs/wk saved"],
                         ["\u23F1 Auto-Detention Billing", "Auto-drafts detention charge after 2 free hours. Owner approves. Stops leaving money on the table.", "$50k-$100k/yr recovered"],
                         ["\uD83D\uDE9A CMMS-Aware Dispatch", "Won\'t assign a truck with expired maintenance or compliance. Prevents DOT violations before they happen.", "$10k-$30k/yr saved"],
                         ["\uD83D\uDCCB HOS FMCSA 2026 Rules", "11/14/60/70 hour rules baked in. Warns dispatchers before violations. Avoids fines.", "$10k+/yr in fines avoided"]
                        ].map(function(item) {
                            return React.createElement("div", { key: item[0], style: { background: C2.orange+"10", border: "1px solid "+C2.orange+"33", borderRadius: 12, padding: "16px 18px", display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 12, flexWrap: "wrap" } },
                                React.createElement("div", { style: { flex: 1 } },
                                    React.createElement("div", { style: { fontSize: 13, fontWeight: 800, color: C2.orange, marginBottom: 4 } }, item[0]),
                                    React.createElement("div", { style: { fontSize: 11, color: C2.dim, lineHeight: 1.7 } }, item[1])),
                                React.createElement("div", { style: { fontSize: 11, fontWeight: 700, color: C2.green, background: C2.green+"18", borderRadius: 7, padding: "4px 10px", whiteSpace: "nowrap", flexShrink: 0 } }, item[2]));
                        })
                    ),
        ...
        React.createElement("div", { style: { padding: "50px 20px", borderBottom: "1px solid "+C2.border, background: "#0A0A0A" } },
            React.createElement("div", { style: { maxWidth: 760, margin: "0 auto" } },
                React.createElement("div", { style: { textAlign: "center", marginBottom: 24 } },
                    React.createElement("div", { style: { fontSize: 11, color: C2.orange, fontWeight: 700, letterSpacing: 2.5, textTransform: "uppercase", marginBottom: 8 } }, "Why This Isn't Expensive"),
                    React.createElement("div", { style: { fontSize: 24, fontWeight: 900, color: C2.white, marginBottom: 8 } }, "Building this from scratch costs $320,000+"),
                    React.createElement("div", { style: { fontSize: 13, color: C2.dim, marginBottom: 20 } }, "The license is a 66%+ discount vs building it yourself, on top of the 90%+ 5-year savings vs Samsara/Motive.")),
                React.createElement("div", { style: { display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(200px,1fr))", gap: 10, marginBottom: 20 } },
                    [["Offline PWA","$50k"],["Voice Dispatch","$30k"],["Auto-Detention","$25k"],["CMMS Dispatch","$20k"],["HOS FMCSA","$15k"],["14 Languages","$50k"],["GPS Tracking","$30k"],["Fleet+Compliance","$40k"],["Payroll+Gusto","$35k"],["CRM+Leads","$25k"]].map(function(item) {
                        return React.createElement("div", { key: item[0], style: { background: C2.card, border: "1px solid "+C2.border, borderRadius: 8, padding: "10px 14px", display: "flex", justifyContent: "space-between" } },
                            React.createElement("span", { style: { fontSize: 11, color: C2.dim } }, item[0]),
...
function Root() {
    ...
    useEffect(function(){
        function goOnline(){ setIsOnline(true); window.dispatchEvent(new CustomEvent("potent-sync")); }
    ...
    useEffect(function(){
        function handleSync(){
            ...
            Promise.all(q.map(function(job){
                ...
                setOfflineQueue(getOfflineQueue()); setSyncing(false);
                if(synced>0){ setSyncMsg(synced+" job"+(synced!==1?"s":"")+" synced!"); setTimeout(function(){setSyncMsg("");},5000); }
                else setSyncMsg("");
            });
        }
        window.addEventListener("potent-sync",handleSync);
        return function(){ window.removeEventListener("potent-sync",handleSync); };
    },[]);
    useEffect(function () {
    ...
    var offlineBanner = React.createElement("div", {style:{
        ...
        display:(!isOnline||syncing||syncMsg||offlineQueue.length>0)?"flex":"none",
        alignItems:"center",justifyContent:"center",gap:8,
        boxShadow:"0 2px 8px rgba(0,0,0,0.4)"
    }},
        !isOnline&&React.createElement("span",null,"Offline Mode - Jobs save locally and sync when you reconnect"+(offlineQueue.length>0?" ("+offlineQueue.length+" queued)":"")),
        isOnline&&syncing&&React.createElement("span",null,syncMsg),
        isOnline&&!syncing&&syncMsg&&React.createElement("span",null,syncMsg),
        isOnline&&!syncing&&!syncMsg&&offlineQueue.length>0&&React.createElement("span",null,
            offlineQueue.length+" job"+(offlineQueue.length!==1?"s":"")+" waiting to sync - ",
            React.createElement("button",{onClick:function(){window.dispatchEvent(new CustomEvent("potent-sync"));},style:{marginLeft:6,background:"#000",color:"#fff",border:"none",borderRadius:5,padding:"3px 8px",fontSize:11,cursor:"pointer",fontFamily:"inherit"}},"Sync Now"))
    );
    if (isAdmin)
        return React.createElement("div",null, offlineBanner, React.createElement(AdminDashboard, { jobs: jobs, onUpdateStatus: updateJob, onAddJob: addJob, onLogout: logout, gasPPG: gasPPG, dieselPPG: dieselPPG, blockedDates: blockedDates, onToggleBlock: toggleBlockDate, role: adminRole, currentUser: currentUser, onApplyAccessorial: addAccessorialToJobPrice }));
    if (showLogin)
...
function EnhancedDriverApp(props) {
    ...
    var [panicSent, setPanicSent] = useState(false);
    ...
    useEffect(function () {
        ...
        watchIdRef.current = navigator.geolocation.watchPosition(function (pos) {
            ...
            // GPS stall detection — runs silently
            if(job && currentUser){checkGPSStall(pos.coords.latitude,pos.coords.longitude,currentUser.id||"driver",currentUser.name||"Driver",job.id);};
        }, function (err) {
    ...
    function sendPanic(){
        if(!latestCoordsRef.current&&!navigator.geolocation){alert("Enable location sharing first.");return;}
        var doSend=function(lat,lng){
            sendEmergencyAlert(currentUser&&currentUser.id?currentUser.id:"driver",currentUser&&currentUser.name?currentUser.name:"Driver",lat,lng,job&&job.id?job.id:null,"PANIC BUTTON PRESSED by driver");
            setPanicSent(true);
            setTimeout(function(){setPanicSent(false);},15000);
        };
        if(latestCoordsRef.current){doSend(latestCoordsRef.current.latitude,latestCoordsRef.current.longitude);}
        else{navigator.geolocation.getCurrentPosition(function(p){doSend(p.coords.latitude,p.coords.longitude);},function(){doSend(0,0);});}
    }
    return React.createElement("div", { style: { maxWidth: 520, margin: "0 auto", paddingBottom: 80 } },
        React.createElement("div",{style:{position:"fixed",bottom:0,left:0,right:0,zIndex:9999,padding:"10px 16px",background:"#000",borderTop:"2px solid "+(panicSent?"#1DB954":"#E53E3E")}},
            React.createElement("button",{onClick:sendPanic,style:{width:"100%",background:panicSent?"#1DB954":"#E53E3E",color:"#fff",border:"none",borderRadius:10,padding:"14px",fontSize:15,fontWeight:900,cursor:"pointer",fontFamily:"inherit",letterSpacing:0.5}},
                panicSent?"✅ ALERT SENT — Stay Safe":"🆘 EMERGENCY — TAP FOR HELP")
        ),
        ...
        job && React.createElement("div", null,
            React.createElement(Card, { style: { marginBottom: 12 } },
                ...
                React.createElement(DriverDocViewer, { jobId: job.id }))),
...
// ── LEAD EMAIL PANEL — handles missing emails gracefully ──────────
function LeadEmailPanel(props){
    var lead=props.lead;
    var emailSending=props.emailSending;
    var emailMsg=props.emailMsg;
    var sendManualEmail=props.sendManualEmail;
    var onSave=props.onSave;
    var [addingEmail,setAddingEmail]=useState(false);
    var [tempEmail,setTempEmail]=useState("");
    ...
    async function saveEmail(){
        ...
        try{
            ...
            setSavedMsg("✅ Email saved — now send a template below");
            setAddingEmail(false);
        }catch(e){setSavedMsg("Failed to save. Try again.");}
        setSaving(false);
    }
    ...
    return React.createElement("div",{style:{background:C.card,border:"1px solid "+C.border,borderRadius:10,padding:"14px 16px",marginBottom:14}},
        ...
        React.createElement("div",{style:{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8}},
            Object.keys(LEAD_EMAIL_TEMPLATES).map(function(key){
                var canSend=!emailSending&&!!lead.email;
                return React.createElement("button",{
                    key:key,
                    disabled:!canSend,
                    onClick:function(){sendManualEmail(lead,key);},
                    style:{border:"1px solid "+(canSend?C.orange:C.border),borderRadius:8,padding:"9px 6px",cursor:canSend?"pointer":"not-allowed",background:canSend?C.orange+"18":C.surface,color:canSend?C.white:C.faint,fontSize:11,fontWeight:700,fontFamily:"inherit",opacity:canSend?1:0.4}
                },LEAD_EMAIL_TEMPLATES[key].label);
            })
        ),
        emailMsg&&React.createElement("div",{style:{fontSize:11,color:emailMsg.indexOf("✅")===0?C.green:C.red,marginTop:8}},emailMsg),
        React.createElement("div",{style:{fontSize:9,color:C.faint,marginTop:8,lineHeight:1.5}},"Auto-sends matching email when you log a lead as Follow-Up or Interested — if email is on file.")
    );
}
    ...
    if (selected) {
        ...
        return React.createElement("div", { style: { maxWidth: 560, margin: "0 auto" } },
            ...
            React.createElement("div", { style: { background: "#F5F1E8", color: "#0B0E14", borderRadius: 12, padding: "20px", marginBottom: 16 } },
                ...
                lead.phone && React.createElement("div", { style: { marginBottom: 12 } },
                    React.createElement("a", { href: "tel:" + lead.phone.replace(/[^\d+]/g, ""), style: { display: "flex", alignItems: "center", gap: 10, background: "#0B0E14", color: "#F5F1E8", borderRadius: 8, padding: "12px 16px", textDecoration: "none", fontWeight: 700, fontSize: 15, marginBottom: 6 } },
                        "\uD83D\uDCDE Call — " + lead.phone),
                    React.createElement("a", { href: "https://voice.google.com/calls?a=nc,+1" + lead.phone.replace(/\D/g,""), target:"_blank", rel:"noopener", style: { display: "flex", alignItems: "center", gap: 10, background: "#0a1f10", color: "#1DB954", borderRadius: 8, padding: "12px 16px", textDecoration: "none", fontWeight: 700, fontSize: 15, border: "1px solid #1DB95444" } },
                        "\uD83D\uDCF2 Call via Google Voice — " + lead.phone)),
...
function LeaderBoard(props) {
    ...
    var [bonusPoints, setBonusPoints] = useState(function(){
        try{return JSON.parse(localStorage.getItem("pl_bonus_pts")||"{}");}catch(e){return {};}
    });
    var [bonusForm, setBonusForm] = useState({name:"",points:"",reason:""});
    var [showBonus, setShowBonus] = useState(false);
    function saveBonusPoints(bp){
        try{localStorage.setItem("pl_bonus_pts",JSON.stringify(bp));}catch(e){}
        setBonusPoints(bp);
    }
    function addBonus(){
        if(!bonusForm.name||!bonusForm.points)return;
        var bp=Object.assign({},bonusPoints);
        bp[bonusForm.name]=(bp[bonusForm.name]||0)+Number(bonusForm.points);
        saveBonusPoints(bp);
        setBonusForm({name:"",points:"",reason:""});
        setShowBonus(false);
    }
    function clearAllBonusPoints(){
    saveBonusPoints({});
}
    ...
    return React.createElement("div", { style: { maxWidth: 760, margin: "0 auto", fontFamily: "'DM Sans','Segoe UI',sans-serif" } },
        ...
        React.createElement("div", { style: { background: "linear-gradient(135deg,#1a1a00,#2a2400)", border: "2px solid " + C.orange, borderRadius: 14, padding: "16px 20px", marginBottom: 20, display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 10 } },
            ...
            isOwner && React.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 6 } },
                ...
                React.createElement("button", { onClick: function(){setShowBonus(!showBonus);}, style: { background: C.orange, color: "#000", border: "none", borderRadius: 7, padding: "7px 12px", fontSize: 11, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" } }, "➕ Add Manual Points"))),
        isOwner && showBonus && React.createElement("div", { style: { background: C.card, border: "1px solid " + C.orange + "44", borderRadius: 10, padding: "14px", marginBottom: 16 } },
            React.createElement("div", { style: { fontSize: 12, fontWeight: 700, color: C.white, marginBottom: 10 } }, "Add Manual Bonus Points — External CRM Calls, Special Recognition"),
            React.createElement("div", { style: { display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 10 } },
                React.createElement("input", { value: bonusForm.name, onChange: function(e){setBonusForm(function(p){return Object.assign({},p,{name:e.target.value});});}, placeholder: "Rep name (e.g. Roberto)", style: { background: C.surface, border: "1px solid " + C.border, borderRadius: 7, color: C.white, padding: "8px 10px", fontSize: 12, outline: "none", fontFamily: "inherit", flex: 1 } }),
                React.createElement("input", { value: bonusForm.points, onChange: function(e){setBonusForm(function(p){return Object.assign({},p,{points:e.target.value});});}, placeholder: "Points (e.g. 500)", type: "number", style: { background: C.surface, border: "1px solid " + C.border, borderRadius: 7, color: C.white, padding: "8px 10px", fontSize: 12, outline: "none", fontFamily: "inherit", width: 100 } }),
                React.createElement("button", { onClick: addBonus, style: { background: C.green, color: "#000", border: "none", borderRadius: 7, padding: "8px 14px", fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" } }, "Add Points")
            ),
            Object.keys(bonusPoints).length > 0 && React.createElement("div", { style: { marginTop: 10 } },
                React.createElement("div", { style: { fontSize: 10, color: C.dim, marginBottom: 6, fontWeight: 700, textTransform: "uppercase", letterSpacing: 0.5 } }, "Bonus Points — included in scores above"),
                Object.entries(bonusPoints).map(function(bpEntry){
                    return React.createElement("div", { key: bpEntry[0], style: { display: "flex", justifyContent: "space-between", alignItems: "center", padding: "6px 8px", background: C.surface, borderRadius: 6, marginBottom: 4 } },
                        React.createElement("span", { style: { fontSize: 11, color: C.white, fontWeight: 700 } }, bpEntry[0]),
                        React.createElement("span", { style: { fontSize: 11, color: C.green, fontWeight: 700 } }, "+" + bpEntry[1].toLocaleString() + " pts included"),
                        React.createElement("button", { onClick: function(){ if(confirm("Remove all bonus points for " + bpEntry[0] + "?")){removeBonus(bpEntry[0]);} }, style: { background: C.red+"22", border: "1px solid "+C.red+"44", borderRadius: 5, color: C.red, cursor: "pointer", fontSize: 10, fontFamily: "inherit", padding: "3px 8px", fontWeight: 700 } }, "Clear")
                    );
                })
            )
        ),
...
// ── FLEET CLOUD SYNC (Supabase + localStorage fallback) ──────────
async function loadFleetFromCloud(){
  if(!navigator.onLine)return{vehicles:loadFleet(),maintenance:loadMaint()};
  try{
    var v=await sbGet("fleet_vehicles","select=*&order=created_at.desc");
    var m=await sbGet("fleet_maintenance","select=*&order=date.desc");
    if(!Array.isArray(v))v=loadFleet(); if(!Array.isArray(m))m=loadMaint();
    var v2=v.map(function(x){return{id:x.id,make:x.make,model:x.model,year:x.year,plate:x.plate,color:x.color||"",vin:x.vin||"",insPolicy:x.ins_policy||"",regExpiry:x.reg_expiry||"",insExpiry:x.ins_expiry||"",oilNext:x.oil_next||"",tireNext:x.tire_next||"",brakeNext:x.brake_next||""};});
    var m2=m.map(function(x){return{id:x.id,vehicleId:x.vehicle_id,type:x.type,date:x.date||"",cost:x.cost||0,shop:x.shop||"",notes:x.notes||""};});
    try{localStorage.setItem(FLEET_KEY,JSON.stringify(v2));localStorage.setItem(MAINT_KEY,JSON.stringify(m2));}catch(e){}
    return{vehicles:v2,maintenance:m2};
  }catch(e){return{vehicles:loadFleet(),maintenance:loadMaint()};}
}
...
async function saveMaintToCloud(rec){
  var payload={id:rec.id,vehicle_id:rec.vehicleId,type:rec.type||"",date:rec.date||null,cost:rec.cost||0,shop:rec.shop||"",notes:rec.notes||""};
  if(navigator.onLine)await sbUpsert("fleet_maintenance",payload);
}

// ── COMPLIANCE CLOUD SYNC ─────────────────────────────────────────
async function loadCompFromCloud(){
  if(!navigator.onLine)return loadComp();
  try{
    var rows=await sbGet("driver_compliance","select=*");
    if(!Array.isArray(rows)||rows.length===0)return loadComp();
    var c={};
    rows.forEach(function(r){c[r.user_id]={cdlNumber:r.cdl_number||"",cdlExpiry:r.cdl_expiry||"",medCardExpiry:r.med_card_expiry||"",mvrDate:r.mvr_date||"",drugTestDate:r.drug_test_date||"",alcoholTestDate:r.alcohol_test_date||"",clearinghouse:r.clearinghouse||"",dotInspDate:r.dot_insp_date||"",dotInspResult:r.dot_insp_result||"",accidentCount:r.accident_count||0,ticketCount:r.ticket_count||0,accidentNotes:r.accident_notes||""};});
    try{localStorage.setItem(COMP_KEY,JSON.stringify(c));}catch(e){}
    return c;
  }catch(e){return loadComp();}
}
async function saveCompToCloud(userId,data){
  var payload={user_id:userId,cdl_number:data.cdlNumber||null,cdl_expiry:data.cdlExpiry||null,med_card_expiry:data.medCardExpiry||null,mvr_date:data.mvrDate||null,drug_test_date:data.drugTestDate||null,alcohol_test_date:data.alcoholTestDate||null,clearinghouse:data.clearinghouse||null,dot_insp_date:data.dotInspDate||null,dot_insp_result:data.dotInspResult||null,accident_count:data.accidentCount||0,ticket_count:data.ticketCount||0,accident_notes:data.accidentNotes||null,updated_at:new Date().toISOString()};
  if(navigator.onLine)await sbUpsert("driver_compliance",payload);
}

// ── EMPLOYEE SETTINGS CLOUD SYNC ─────────────────────────────────
async function loadEmpSettingsFromCloud(){
  if(!navigator.onLine){try{return JSON.parse(localStorage.getItem("pl_emp_settings")||"{}");}catch(e){return{};}}
  try{
    var rows=await sbGet("employee_settings","select=*");
    if(!Array.isArray(rows)||rows.length===0){try{return JSON.parse(localStorage.getItem("pl_emp_settings")||"{}");}catch(e){return{};}}
    var s={};
    rows.forEach(function(r){s[r.user_id]={displayName:r.display_name||"",hireDate:r.hire_date||"",status:r.status||"active",payType:r.pay_type||"commission",hourlyRate:r.hourly_rate||0,salary:r.salary||0,logisticsRate:r.logistics_rate||10,osRate:r.os_rate||10,loadboardRate:r.loadboard_rate||10,bankName:r.bank_name||"",routingNum:r.routing_num||"",accountNum:r.account_num||""};});
    try{localStorage.setItem("pl_emp_settings",JSON.stringify(s));}catch(e){}
    return s;
  }catch(e){try{return JSON.parse(localStorage.getItem("pl_emp_settings")||"{}");}catch(e2){return{};}}
}
async function saveEmpSettingToCloud(userId,data){
  try{var s=JSON.parse(localStorage.getItem("pl_emp_settings")||"{}");s[userId]=Object.assign(s[userId]||{},data);localStorage.setItem("pl_emp_settings",JSON.stringify(s));}catch(e){}
  if(!navigator.onLine)return;
  var payload={user_id:userId,display_name:data.displayName||null,hire_date:data.hireDate||null,status:data.status||"active",pay_type:data.payType||"commission",hourly_rate:data.hourlyRate||null,salary:data.salary||null,logistics_rate:data.logisticsRate||10,os_rate:data.osRate||10,loadboard_rate:data.loadboardRate||10,bank_name:data.bankName||null,routing_num:data.routingNum||null,account_num:data.accountNum||null,updated_at:new Date().toISOString()};
  await sbUpsert("employee_settings",payload);
}
...
function ComplianceTracker(props) {
    ...
    useEffect(function(){
        loadCompFromCloud().then(function(data){
            if(data&&Object.keys(data).length>0)setComp(data);
        });
    },[]);
    var [selected, setSelected] = useState(null);
...
// === POTENT OS SALES PIPELINE & TRAINING ===
var STAGES = {
    cold_1: { label: "Cold Email 1", color: "#4299E1", next: "followup_1" },
    followup_1: { label: "Follow-Up 1 (Day 3)", color: "#4299E1", next: "followup_2" },
    followup_2: { label: "Follow-Up 2 (Day 7)", color: "#4299E1", next: "followup_3" },
    followup_3: { label: "Follow-Up 3 (Day 14)", color: "#F0E000", next: "followup_4" },
    followup_4: { label: "Follow-Up 4 (Day 21)", color: "#F0E000", next: "nurture" },
    warm_2: { label: "Warm Email 2 (Replied)", color: "#ED8936", next: "hot_3" },
    hot_3: { label: "Hot Email 3 (Booked)", color: "#ED8936", next: "zoom" },
    zoom: { label: "Zoom Scheduled", color: "#9F7AEA", next: "closed_won" },
    closed_won: { label: "Closed Won", color: "#1DB954", next: null },
    closed_lost: { label: "Closed Lost", color: "#E53E3E", next: null },
...
function getVersion(fs) { if (!fs || fs < 1)
    ...
    return "C"; if (fs <= 150)
    return "B"; return "A"; }
function getTemplate(ver, stage) { var k = ver + "_" + stage; if (EMAIL_TEMPLATES[k])
    return EMAIL_TEMPLATES[k]; if (EMAIL_TEMPLATES[stage])
...
function OSPipeline(props) {
    ...
    var [form, setForm] = useState({ company_name: "", contact_name: "", email: "", phone: "", fleet_size: "", current_software: "Motive/Samsara", monthly_gps_cost: "", state: "", notes: "", stage: "cold_1", next_followup: "", assigned_to: currentUser ? currentUser.name : "" });
    function sbOS(method, data, qs) {
    ...
    async function saveP() {
        if (!form.company_name || !form.contact_name) {
            setMsg("Company and contact required.");
            return;
        }
        setSaving(true);
        setMsg("");
        var ver = getVersion(Number(form.fleet_size) || 0);
        var payload = Object.assign({}, form, { fleet_size: Number(form.fleet_size) || null, monthly_gps_cost: Number(form.monthly_gps_cost) || null, version: ver, last_contact: new Date().toISOString().slice(0, 10), updated_at: new Date().toISOString() });
        try {
            var res = await sbOS("POST", [payload]);
            if (Array.isArray(res) && res[0])
                setProspects(function (p) { return [res[0]].concat(p); });
            setView("board");
        }
        catch (e) {
    ...
    async function updateStage(p, s) {
        var up = { stage: s, updated_at: new Date().toISOString(), last_contact: new Date().toISOString().slice(0, 10) };
        await fetch(SUPABASE_URL + "/rest/v1/os_prospects?id=eq." + p.id, { method: "PATCH", headers: { apikey: SUPABASE_ANON_KEY, Authorization: "Bearer " + SUPABASE_ANON_KEY, "Content-Type": "application/json", Prefer: "return=representation" }, body: JSON.stringify(up) });
        setProspects(function (pr) { return pr.map(function (x) { return x.id === p.id ? Object.assign({}, x, up) : x; }); });
    ...
    async function closeDeal(p) {
        ...
        var up = { deal_value: Number(val) || null, close_date: dt || null, stage: "closed_won", updated_at: new Date().toISOString() };
        await fetch(SUPABASE_URL + "/rest/v1/os_prospects?id=eq." + p.id, { method: "PATCH", headers: { apikey: SUPABASE_ANON_KEY, Authorization: "Bearer " + SUPABASE_ANON_KEY, "Content-Type": "application/json", Prefer: "return=representation" }, body: JSON.stringify(up) });
        setProspects(function (pr) { return pr.map(function (x) { return x.id === p.id ? Object.assign({}, x, up) : x; }); });
        if (selected && selected.id === p.id)
            setSelected(function (sx) { return Object.assign({}, sx, up); });
    }
    function copyTmpl(p, stage) {
        var ver = p.version || getVersion(Number(p.fleet_size) || 0);
        var tmpl = getTemplate(ver, stage);
        if (!tmpl) {
            alert("No template for this stage.");
            return;
        }
        var body = tmpl.body.replace(/\[First Name\]/g, p.contact_name.split(" ")[0]).replace(/\[Fleet\]/g, p.fleet_size || "your");
        copyText("Subject: " + tmpl.subject + "\n\n" + body);
        setCopied(stage);
        setTimeout(function () { setCopied(""); }, 2500);
    }
    var filtered = prospects.filter(function (p) { return stageFilter === "all" || p.stage === stageFilter; });
    var wonRev = prospects.filter(function (p) { return p.stage === "closed_won" && p.deal_value; }).reduce(function (s, p) { return s + (p.deal_value || 0); }, 0);
    var dueToday = prospects.filter(function (p) { return p.next_followup && p.next_followup <= new Date().toISOString().slice(0, 10) && p.stage !== "closed_won" && p.stage !== "closed_lost"; }).length;
    if (view === "add")
        return React.createElement("div", { style: { maxWidth: 600, margin: "0 auto" } },
            React.createElement("div", { style: { display: "flex", gap: 10, alignItems: "center", marginBottom: 16 } },
                React.createElement(Btn, { onClick: function () { setView("board"); }, variant: "ghost", style: { fontSize: 12, padding: "6px 12px" } }, "\u2190 Back"),
                React.createElement("div", { style: { fontSize: 16, fontWeight: 900, color: C.white } }, "Add Prospect")),
            msg && React.createElement("div", { style: { color: C.red, fontSize: 12, marginBottom: 10 } }, msg),
            React.createElement(Card, null,
                React.createElement("div", { style: { display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0 12px" } },
                    [["company_name", "Company Name", "text", 2], ["contact_name", "Contact Name", "text", 1], ["email", "Email", "email", 1], ["phone", "Phone", "tel", 1], ["fleet_size", "Fleet Size (trucks)", "number", 1], ["current_software", "Current Software", "text", 1], ["monthly_gps_cost", "Monthly GPS Cost ($)", "number", 1], ["state", "State", "text", 1]].map(function (f) {
                        return React.createElement("div", { key: f[0], style: { marginBottom: 10, gridColumn: f[3] === 2 ? "1/-1" : "auto" } },
                            React.createElement("div", { style: { fontSize: 9, color: C.dim, textTransform: "uppercase", marginBottom: 3 } }, f[1]),
                            React.createElement("input", { type: f[2], value: form[f[0]] || "", onChange: function (e) { var v = e.target.value; setF(f[0], v); }, style: { background: C.surface, border: "1px solid " + C.border, borderRadius: 7, color: C.white, padding: "8px 10px", fontSize: 12, width: "100%", outline: "none", fontFamily: "inherit", boxSizing: "border-box" } }));
                    }),
                    React.createElement("div", { style: { marginBottom: 10, gridColumn: "1/-1" } },
                        React.createElement("div", { style: { fontSize: 9, color: C.dim, textTransform: "uppercase", marginBottom: 3 } }, "Stage"),
                        React.createElement("select", { value: form.stage, onChange: function (e) { setF("stage", e.target.value); }, style: { background: C.surface, border: "1px solid " + C.border, borderRadius: 7, color: C.white, padding: "8px 10px", fontSize: 12, width: "100%", outline: "none", fontFamily: "inherit", colorScheme: "dark" } }, Object.entries(STAGES).map(function (s) { return React.createElement("option", { key: s[0], value: s[0] }, s[1].label); }))),
                    React.createElement("div", { style: { marginBottom: 10 } },
                        React.createElement("div", { style: { fontSize: 9, color: C.dim, textTransform: "uppercase", marginBottom: 3 } }, "Next Follow-Up"),
                        React.createElement("input", { type: "date", value: form.next_followup || "", onChange: function (e) { setF("next_followup", e.target.value); }, style: { background: C.surface, border: "1px solid " + C.border, borderRadius: 7, color: C.white, padding: "8px 10px", fontSize: 12, width: "100%", outline: "none", fontFamily: "inherit", colorScheme: "dark" } })),
                    React.createElement("div", { style: { marginBottom: 10 } },
                        React.createElement("div", { style: { fontSize: 9, color: C.dim, textTransform: "uppercase", marginBottom: 3 } }, "Assigned To"),
                        React.createElement("input", { value: form.assigned_to || "", onChange: function (e) { setF("assigned_to", e.target.value); }, style: { background: C.surface, border: "1px solid " + C.border, borderRadius: 7, color: C.white, padding: "8px 10px", fontSize: 12, width: "100%", outline: "none", fontFamily: "inherit", boxSizing: "border-box" } })),
                    React.createElement("div", { style: { marginBottom: 10, gridColumn: "1/-1" } },
                        React.createElement("div", { style: { fontSize: 9, color: C.dim, textTransform: "uppercase", marginBottom: 3 } }, "Notes"),
                        React.createElement("textarea", { value: form.notes || "", onChange: function (e) { setF("notes", e.target.value); }, rows: 3, style: { background: C.surface, border: "1px solid " + C.border, borderRadius: 7, color: C.white, padding: "8px 10px", fontSize: 12, width: "100%", outline: "none", fontFamily: "inherit", boxSizing: "border-box", resize: "vertical" } }))),
                form.fleet_size && React.createElement("div", { style: { background: C.orange + "18", border: "1px solid " + C.orange + "44", borderRadius: 8, padding: "10px 12px", marginBottom: 10, fontSize: 12 } },
                    React.createElement("span", { style: { color: C.dim } }, "Version: "),
                    React.createElement("strong", { style: { color: C.orange } },
                        "Version ",
                        getVersion(Number(form.fleet_size))),
                    React.createElement("span", { style: { color: C.dim, marginLeft: 12 } }, "License: "),
                    React.createElement("strong", { style: { color: C.green } }, getLicensePrice(Number(form.fleet_size)))),
                React.createElement(Btn, { onClick: saveP, disabled: saving, style: { width: "100%", padding: "11px" } }, saving ? "Saving..." : "+ Add Prospect")));
    if (view === "detail" && selected) {
        var ver2 = selected.version || getVersion(Number(selected.fleet_size) || 0);
        var stgInfo = STAGES[selected.stage] || { label: selected.stage, color: C.dim };
        var isWon = selected.stage === "closed_won";
        return React.createElement("div", { style: { maxWidth: 680, margin: "0 auto" } },
            React.createElement("div", { style: { display: "flex", gap: 10, alignItems: "center", marginBottom: 14, flexWrap: "wrap" } },
                React.createElement(Btn, { onClick: function () { setView("board"); }, variant: "ghost", style: { fontSize: 12, padding: "6px 12px" } }, "\u2190 Pipeline"),
                React.createElement("div", { style: { fontSize: 16, fontWeight: 900, color: C.white, flex: 1 } }, selected.company_name),
                React.createElement("div", { style: { background: stgInfo.color + "22", color: stgInfo.color, borderRadius: 7, padding: "4px 10px", fontSize: 10, fontWeight: 700 } }, stgInfo.label)),
            React.createElement("div", { style: { display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(140px,1fr))", gap: 8, marginBottom: 14 } }, [["Contact", selected.contact_name], ["Email", selected.email || "—"], ["Phone", selected.phone || "—"], ["Fleet", (selected.fleet_size || "?") + (" trucks")], ["Software", selected.current_software || "—"], ["Monthly GPS", selected.monthly_gps_cost ? "$" + Number(selected.monthly_gps_cost).toLocaleString() : "—"], ["License", getLicensePrice(selected.fleet_size) || "—"], ["Version", "Ver " + (ver2 || "?")], ["Follow-Up", selected.next_followup || "—"], ["Last Contact", selected.last_contact || "—"]].map(function (item) {
                return React.createElement("div", { key: item[0], style: { background: C.card, border: "1px solid " + C.border, borderRadius: 8, padding: "10px 12px" } },
                    React.createElement("div", { style: { fontSize: 9, color: C.dim, textTransform: "uppercase", marginBottom: 2 } }, item[0]),
                    React.createElement("div", { style: { fontSize: 12, fontWeight: 600, color: C.white } }, item[1]));
            })),
            selected.notes && React.createElement(Card, { style: { marginBottom: 12 } },
                React.createElement("div", { style: { fontSize: 9, color: C.dim, textTransform: "uppercase", marginBottom: 3 } }, "Notes"),
                React.createElement("div", { style: { fontSize: 12, color: C.dim, lineHeight: 1.7 } }, selected.notes)),
            React.createElement(Card, null,
                React.createElement("div", { style: { fontSize: 12, fontWeight: 700, color: C.white, marginBottom: 10 } }, "\uD83D\uDCE7 Email Templates \u2014 One-Click Copy"),
                React.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 8 } }, Object.keys(STAGES).filter(function (s) { return s !== "closed_won" && s !== "closed_lost" && s !== "nurture"; }).map(function (s) {
                    var tmpl = getTemplate(ver2, s);
                    if (!tmpl)
                        return null;
                    return React.createElement("div", { key: s, style: { display: "flex", justifyContent: "space-between", alignItems: "center", background: C.surface, borderRadius: 7, padding: "8px 12px" } },
                        React.createElement("div", null,
                            React.createElement("div", { style: { fontSize: 11, fontWeight: 700, color: selected.stage === s ? C.orange : C.white } }, STAGES[s]?.label || s),
                            React.createElement("div", { style: { fontSize: 10, color: C.dim } },
                                "Subj: ",
                                tmpl.subject)),
                        React.createElement("button", { onClick: function () { copyTmpl(selected, s); }, style: { background: copied === s ? C.green : C.orange, color: "#000", border: "none", borderRadius: 6, padding: "5px 10px", fontSize: 11, fontWeight: 700, cursor: "pointer", fontFamily: "inherit", whiteSpace: "nowrap", flexShrink: 0 } }, copied === s ? "✓ Copied!" : "Copy →"));
                }))),
            React.createElement(Card, null,
                React.createElement("div", { style: { fontSize: 12, fontWeight: 700, color: C.white, marginBottom: 10 } }, "\uD83D\uDD04 Update Stage"),
                React.createElement("div", { style: { display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(130px,1fr))", gap: 6 } }, Object.entries(STAGES).map(function (s) {
                    return React.createElement("button", { key: s[0], onClick: function () { updateStage(selected, s[0]); }, style: { background: selected.stage === s[0] ? s[1].color + "33" : "transparent", border: "1px solid " + (selected.stage === s[0] ? s[1].color : C.border), borderRadius: 7, padding: "7px 8px", cursor: "pointer", color: selected.stage === s[0] ? s[1].color : C.dim, fontSize: 10, fontWeight: 700, fontFamily: "inherit", textAlign: "center" } }, s[1].label);
                }))),
            !isWon && React.createElement(Card, null,
                React.createElement("div", { style: { fontSize: 12, fontWeight: 700, color: C.white, marginBottom: 8 } }, "Close Deal"),
                React.createElement(Btn, { onClick: function () { closeDeal(selected); }, style: { fontSize: 12, padding: "8px 16px" } }, "\uD83C\uDF89 Mark as Closed Won")),
            isWon && React.createElement("div", { style: { background: C.green + "18", border: "1px solid " + C.green + "44", borderRadius: 10, padding: 14, textAlign: "center" } },
                React.createElement("div", { style: { fontSize: 16, fontWeight: 900, color: C.green } }, "CLOSED WON"),
                selected.deal_value && React.createElement("div", { style: { fontSize: 22, fontWeight: 900, color: C.white, marginTop: 4 } },
                    "$",
                    Number(selected.deal_value).toLocaleString()),
                selected.close_date && React.createElement("div", { style: { fontSize: 11, color: C.dim, marginTop: 2 } },
                    "Closed ",
                    selected.close_date)));
    }
    return React.createElement("div", { style: { maxWidth: 860, margin: "0 auto" } },
        React.createElement("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14, flexWrap: "wrap", gap: 8 } },
            React.createElement("div", { style: { fontSize: 18, fontWeight: 900, color: C.white } }, "\uD83C\uDFAF POTENT OS Sales Pipeline"),
            React.createElement(Btn, { onClick: function () { setView("add"); }, style: { fontSize: 12, padding: "8px 14px" } }, "+ Add Prospect")),
        React.createElement("div", { style: { display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(130px,1fr))", gap: 10, marginBottom: 14 } }, [["Total", prospects.length, C.white], ["Active", prospects.filter(function (p) { return p.stage !== "closed_won" && p.stage !== "closed_lost" && p.stage !== "nurture"; }).length, C.blue], ["Due Today", dueToday, dueToday > 0 ? C.red : C.green], ["Revenue Won", "$" + wonRev.toLocaleString(), C.green], ["Deals Won", prospects.filter(function (p) { return p.stage === "closed_won"; }).length, C.green]].map(function (k) {
            return React.createElement("div", { key: k[0], style: { background: C.card, border: "1px solid " + C.border, borderRadius: 10, padding: "12px 14px" } },
                React.createElement("div", { style: { fontSize: 9, color: C.dim, textTransform: "uppercase", marginBottom: 2 } }, k[0]),
                React.createElement("div", { style: { fontSize: 18, fontWeight: 900, color: k[2] } }, k[1]));
        })),
        ...
        filtered.map(function (p) {
            ...
            var stg2 = STAGES[p.stage] || { label: p.stage, color: C.dim };
            var overdue = p.next_followup && p.next_followup < new Date().toISOString().slice(0, 10) && p.stage !== "closed_won" && p.stage !== "closed_lost";
            return React.createElement("div", { key: p.id, onClick: function () { setSelected(p); setView("detail"); }, style: { background: C.card, border: "1px solid " + (overdue ? C.red : C.border), borderRadius: 10, padding: "12px 16px", marginBottom: 8, cursor: "pointer" } },
                React.createElement("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 12, flexWrap: "wrap" } },
                    React.createElement("div", { style: { flex: 1 } },
                        React.createElement("div", { style: { display: "flex", gap: 8, alignItems: "center", marginBottom: 4, flexWrap: "wrap" } },
                            React.createElement("div", { style: { fontSize: 14, fontWeight: 700, color: C.white } }, p.company_name),
                            ver3 && React.createElement("span", { style: { fontSize: 9, background: "#9F7AEA22", color: "#9F7AEA", borderRadius: 5, padding: "2px 7px", fontWeight: 700 } },
                                "Ver ",
                                ver3),
                            getLicensePrice(p.fleet_size) && React.createElement("span", { style: { fontSize: 9, background: C.green + "18", color: C.green, borderRadius: 5, padding: "2px 7px", fontWeight: 700 } }, getLicensePrice(p.fleet_size))),
                        React.createElement("div", { style: { fontSize: 11, color: C.dim } },
                            p.contact_name,
                            p.fleet_size ? " · " + p.fleet_size + " trucks" : ""),
                        p.next_followup && React.createElement("div", { style: { fontSize: 10, color: overdue ? C.red : C.dim, marginTop: 3 } },
                            "Follow-up: ",
                            p.next_followup,
                            overdue ? " OVERDUE" : "")),
                    React.createElement("div", { style: { display: "flex", gap: 8, alignItems: "center", flexShrink: 0 } },
                        p.stage === "closed_won" && p.deal_value && React.createElement("div", { style: { fontSize: 14, fontWeight: 900, color: C.green } },
                            "$",
                            Number(p.deal_value).toLocaleString()),
                        React.createElement("div", { style: { background: stg2.color + "22", color: stg2.color, borderRadius: 7, padding: "4px 10px", fontSize: 10, fontWeight: 700, whiteSpace: "nowrap" } }, stg2.label),
                        React.createElement("div", { style: { color: C.dim } }, "\u203A"))));
        }));
}
function OSTraining() {
    var [sec, setSec] = useState("sake");
    var [copied, setCopied] = useState("");
    var [qa, setQa] = useState({});
    var [submitted, setSubmitted] = useState(false);
    var SECS = [{ id: "sake", label: "SAKE" }, { id: "flow", label: "Email Flow" }, { id: "vA", label: "Version A" }, { id: "vB", label: "Version B" }, { id: "vC", label: "Version C" }, { id: "fu", label: "Follow-Ups" }, { id: "zoom", label: "Zoom Script" }, { id: "quiz", label: "Quiz" }];
    function copyE(subject, body, k) {
        copyText("Subject: " + subject + "\n\n" + body);
        setCopied(k);
        setTimeout(function () { setCopied(""); }, 2500);
    }
    function EBlock({ title, subject, body, ck }) {
        return React.createElement("div", { style: { background: C.surface, border: "1px solid " + C.border, borderRadius: 9, padding: "12px", marginBottom: 10 } },
            React.createElement("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 } },
                React.createElement("div", { style: { fontSize: 12, fontWeight: 700, color: C.white } }, title),
                React.createElement("button", { onClick: function () { copyE(subject, body, ck); }, style: { background: copied === ck ? C.green : C.orange, color: "#000", border: "none", borderRadius: 6, padding: "5px 12px", fontSize: 11, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" } }, copied === ck ? "✓ Copied!" : "Copy")),
            React.createElement("div", { style: { fontSize: 10, color: C.orange, marginBottom: 5 } },
                "Subject: ",
                subject),
            React.createElement("pre", { style: { fontSize: 11, color: C.dim, lineHeight: 1.7, whiteSpace: "pre-wrap", fontFamily: "inherit", margin: 0, maxHeight: 180, overflow: "auto" } }, body));
    }
    var QUIZ = [
        { q: "What does SAKE stand for?", opts: ["Situation, Amplify, Key solution, End with question", "Sales, Approach, Knowledge, Execute", "Start, Ask, Know, Engage", "Sell, Analyze, Keep, Expand"], c: 0 },
        { q: "Which version do you send a 75-truck fleet?", opts: ["Version A", "Version B", "Version C", "All versions"], c: 1 },
        { q: "What is the first thing you ask in Email 1?", opts: ["Can I schedule a demo?", "What are you currently paying per truck?", "Do you want to switch?", "What is your budget?"], c: 1 },
    ...
    return React.createElement("div", { style: { maxWidth: 760, margin: "0 auto" } },
        React.createElement("div", { style: { fontSize: 18, fontWeight: 900, color: C.white, marginBottom: 4 } }, "\uD83D\uDCDA POTENT OS Sales Training"),
        React.createElement("div", { style: { fontSize: 11, color: C.dim, marginBottom: 14 } }, "Master the email sequence, SAKE framework, and Zoom close."),
        React.createElement("div", { style: { display: "flex", gap: 4, flexWrap: "wrap", marginBottom: 18 } }, SECS.map(function (s) { return React.createElement("button", { key: s.id, onClick: function () { setSec(s.id); }, style: { background: sec === s.id ? C.orange : "transparent", color: sec === s.id ? "#000" : C.dim, border: "1px solid " + (sec === s.id ? C.orange : C.border), borderRadius: 6, padding: "5px 10px", fontSize: 10, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" } }, s.label); })),
        sec === "sake" && React.createElement("div", null,
        ...
        sec === "flow" && React.createElement("div", null,
            ...
            [["COLD EMAIL 1", "Wedge + Offline Mode + Price + Question", "No reply → Follow-Up Day 3, 7, 14, 21. Reply → Warm Email 2 immediately"],
                ...
                return React.createElement("div", { key: s[0], style: { background: C.card, border: "1px solid " + C.border, borderRadius: 10, padding: "14px", marginBottom: 10 } },
                    React.createElement("div", { style: { fontSize: 13, fontWeight: 800, color: C.orange, marginBottom: 2 } }, s[0]),
                    React.createElement("div", { style: { fontSize: 11, color: C.dim, marginBottom: 6 } }, s[1]),
                    React.createElement("div", { style: { fontSize: 11, color: C.green } },
                        "\u2192 ",
                        s[2]));
            })),
        ...
        sec === "fu" && React.createElement("div", null,
            React.createElement("div", { style: { fontSize: 12, color: C.dim, marginBottom: 12 } }, "Use these when they don't reply to Email 1. All versions use the same follow-up sequence."),
            React.createElement(EBlock, { title: "Follow-Up 1 \u2014 Day 3", subject: EMAIL_TEMPLATES.followup_1.subject, body: EMAIL_TEMPLATES.followup_1.body, ck: "FU1" }),
            React.createElement(EBlock, { title: "Follow-Up 2 \u2014 Day 7", subject: EMAIL_TEMPLATES.followup_2.subject, body: EMAIL_TEMPLATES.followup_2.body, ck: "FU2" }),
            React.createElement(EBlock, { title: "Follow-Up 3 \u2014 Day 14", subject: EMAIL_TEMPLATES.followup_3.subject, body: EMAIL_TEMPLATES.followup_3.body, ck: "FU3" }),
            React.createElement(EBlock, { title: "Follow-Up 4 \u2014 Day 21 (Last)", subject: EMAIL_TEMPLATES.followup_4.subject, body: EMAIL_TEMPLATES.followup_4.body, ck: "FU4" })),
        sec === "zoom" && React.createElement("div", null,
            ...
            [["S — Situation", "\"What's the most frustrating thing about your GPS right now?\"", "Listen. Do not pitch yet."],
                ...
                return React.createElement("div", { key: s[0], style: { background: C.card, border: "1px solid " + C.border, borderRadius: 10, padding: "14px", marginBottom: 10 } },
                    React.createElement("div", { style: { fontSize: 12, fontWeight: 800, color: C.orange, marginBottom: 5 } }, s[0]),
                    React.createElement("div", { style: { background: C.surface, borderRadius: 7, padding: "10px 12px", marginBottom: 5, fontSize: 12, color: C.white, fontStyle: "italic", lineHeight: 1.7 } }, s[1]),
                    React.createElement("div", { style: { fontSize: 11, color: C.dim } }, s[2]));
            })),
        sec === "quiz" && React.createElement("div", null,
            React.createElement("div", { style: { fontSize: 13, fontWeight: 700, color: C.white, marginBottom: 12 } },
                "Knowledge Check \u2014 ",
                QUIZ.length,
                " Questions"),
            !submitted ? React.createElement("div", null,
                QUIZ.map(function (q, qi) {
                    return React.createElement("div", { key: qi, style: { background: C.card, border: "1px solid " + C.border, borderRadius: 10, padding: "14px", marginBottom: 10 } },
                        React.createElement("div", { style: { fontSize: 12, fontWeight: 700, color: C.white, marginBottom: 10 } },
                            "Q",
                            qi + 1,
                            ". ",
                        ...
                        q.opts.map(function (opt, oi) {
                            return React.createElement("div", { key: oi, onClick: function () { setQa(function (p) { var n = Object.assign({}, p); n[qi] = oi; return n; }); }, style: { display: "flex", gap: 10, alignItems: "center", padding: "8px 10px", marginBottom: 5, background: qa[qi] === oi ? C.orange + "22" : C.surface, border: "1px solid " + (qa[qi] === oi ? C.orange : C.border), borderRadius: 7, cursor: "pointer" } },
                                React.createElement("div", { style: { width: 14, height: 14, borderRadius: "50%", border: "2px solid " + (qa[qi] === oi ? C.orange : C.dim), background: qa[qi] === oi ? C.orange : "transparent", flexShrink: 0 } }),
                                React.createElement("span", { style: { fontSize: 12, color: qa[qi] === oi ? C.white : C.dim } }, opt));
                        }));
                }),
                React.createElement(Btn, { onClick: function () { if (Object.keys(qa).length < QUIZ.length) {
                        alert("Answer all questions.");
                        return;
                    } setSubmitted(true); }, style: { width: "100%", padding: "12px" } }, "Submit Quiz")) : React.createElement("div", null,
                React.createElement("div", { style: { textAlign: "center", padding: "24px 0", marginBottom: 16 } },
                    React.createElement("div", { style: { fontSize: 48, fontWeight: 900, color: score >= 6 ? C.green : C.red } },
                        score,
                        "/",
                        QUIZ.length),
                    React.createElement("div", { style: { fontSize: 14, color: score >= 6 ? C.green : C.orange, fontWeight: 700, marginTop: 4 } }, score >= 6 ? "Passed! You're ready." : "Review training and try again.")),
                QUIZ.map(function (q, qi) {
                    var ok = qa[qi] === q.c;
                    return React.createElement("div", { key: qi, style: { background: C.card, border: "1px solid " + (ok ? C.green : C.red) + "44", borderRadius: 9, padding: "12px", marginBottom: 7 } },
                        React.createElement("div", { style: { fontSize: 11, fontWeight: 700, color: ok ? C.green : C.red, marginBottom: 3 } },
                            ok ? "✅" : "❌",
                            " Q",
                            qi + 1,
                            ". ",
                        ...
                        React.createElement("div", { style: { fontSize: 11, color: C.dim } },
                            "Answer: ",
                            React.createElement("strong", { style: { color: C.white } }, q.opts[q.c])));
                }),
                React.createElement(Btn, { onClick: function () { setSubmitted(false); setQa({}); }, variant: "ghost", style: { width: "100%", padding: "11px", marginTop: 8 } }, "Retake Quiz"))));
}

// ═══════════════════════════════════════════════════════════════════
// 🔒 SAFETY DASHBOARD — OWNER ONLY — INVISIBLE TO DRIVERS
// ═══════════════════════════════════════════════════════════════════
function SafetyDashboard(props){
  var [config,setConfigState]=useState(loadSafetyConfig);
  var [alerts,setAlerts]=useState([]);
  var [loading,setLoading]=useState(true);
  var [activeTab,setActiveTab]=useState("alerts");

  // Voice monitoring removed for privacy
  var users=props.users||[];
  var jobs=props.jobs||[];
  ...
  function loadAlerts(){
    setLoading(true);
    sbGet("safety_alerts","select=*&order=created_at.desc&limit=100").then(function(data){
  ...
  async function acknowledge(id){
    await fetch(SUPABASE_URL+"/rest/v1/safety_alerts?id=eq."+id,{
      method:"PATCH",
      headers:{apikey:SUPABASE_ANON_KEY,Authorization:"Bearer "+SUPABASE_ANON_KEY,"Content-Type":"application/json"},
      body:JSON.stringify({acknowledged:true,acknowledged_at:new Date().toISOString(),acknowledged_by:"Owner"})
    });
    setAlerts(function(a){return a.map(function(x){return x.id===id?Object.assign({},x,{acknowledged:true}):x;});});
  }
  ...
  function sendTestAlert(){
    if(navigator.geolocation){
      navigator.geolocation.getCurrentPosition(function(pos){
        sendEmergencyAlert("test","Test Alert",pos.coords.latitude,pos.coords.longitude,"TEST-001","TEST ALERT — admin triggered manual test");
        setTimeout(loadAlerts,1000);
      });
    }
  }

  var unacked=alerts.filter(function(a){return !a.acknowledged;}).length;

  return React.createElement("div",{style:{maxWidth:760,margin:"0 auto"}},
    // Header
    React.createElement("div",{style:{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14,flexWrap:"wrap",gap:8}},
      React.createElement("div",null,
        React.createElement("div",{style:{fontSize:18,fontWeight:900,color:C.white}},"🔒 Safety & Security Dashboard"),
        React.createElement("div",{style:{fontSize:10,color:C.red,fontWeight:700,marginTop:2}},"OWNER ONLY — NOT VISIBLE TO DRIVERS OR DISPATCHERS")
      ),
      unacked>0&&React.createElement("div",{style:{background:C.red+"22",color:C.red,border:"1px solid "+C.red+"44",borderRadius:8,padding:"6px 14px",fontSize:12,fontWeight:700}},
    ...
    React.createElement("div",{style:{display:"flex",gap:4,marginBottom:16}},
      [["alerts","🚨 Alerts"+(unacked>0?" ("+unacked+")":"")],["config","⚙ Config"]].map(function(t){
        return React.createElement("button",{key:t[0],onClick:function(){setActiveTab(t[0]);},style:{background:activeTab===t[0]?C.red:"transparent",color:activeTab===t[0]?"#fff":C.dim,border:"1px solid "+(activeTab===t[0]?C.red:C.border),borderRadius:6,padding:"6px 12px",fontSize:11,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}},t[1]);
      })
    ),
    ...
    activeTab==="alerts"&&React.createElement("div",null,
      React.createElement("div",{style:{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}},
        React.createElement("div",{style:{fontSize:12,color:C.dim}},"Auto-refreshes every 30 seconds. All times are local."),
        React.createElement("div",{style:{display:"flex",gap:8}},
          React.createElement("button",{onClick:sendTestAlert,style:{background:"transparent",border:"1px solid "+C.dim,color:C.dim,borderRadius:6,padding:"5px 10px",fontSize:10,cursor:"pointer",fontFamily:"inherit"}},"Send Test Alert"),
          React.createElement("button",{onClick:loadAlerts,style:{background:"transparent",border:"1px solid "+C.border,color:C.dim,borderRadius:6,padding:"5px 10px",fontSize:10,cursor:"pointer",fontFamily:"inherit"}},"Refresh")
        )
      ),
      loading&&React.createElement("div",{style:{textAlign:"center",padding:30,color:C.dim}},"Loading alerts..."),
      !loading&&alerts.length===0&&React.createElement("div",{style:{textAlign:"center",padding:40,color:C.dim,fontSize:13}},"No alerts. System is running clean."),
      alerts.map(function(a){
        return React.createElement("div",{key:a.id,style:{background:a.acknowledged?C.card:C.red+"18",border:"1px solid "+(a.acknowledged?C.border:C.red),borderRadius:10,padding:"12px 16px",marginBottom:8}},
          React.createElement("div",{style:{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:12,flexWrap:"wrap"}},
            React.createElement("div",{style:{flex:1}},
              React.createElement("div",{style:{display:"flex",gap:8,alignItems:"center",marginBottom:4,flexWrap:"wrap"}},
                React.createElement("div",{style:{fontSize:13,fontWeight:700,color:a.acknowledged?C.dim:C.red}},"🚨 "+a.reason),
                a.acknowledged&&React.createElement("div",{style:{fontSize:9,color:C.green,background:C.green+"18",borderRadius:5,padding:"2px 7px",fontWeight:700}},"ACK")
              ),
              React.createElement("div",{style:{fontSize:11,color:C.dim,marginBottom:4}},
                "Driver: "+(a.driver_name||a.driver_id)+" · Job: "+(a.job_id||"N/A")+" · "+new Date(a.created_at).toLocaleString()
              ),
              a.lat&&React.createElement("a",{href:"https://maps.google.com/?q="+a.lat+","+a.lng,target:"_blank",rel:"noopener",style:{fontSize:11,color:C.blue,textDecoration:"none",fontWeight:700}},"📍 View on Map →")
            ),
            !a.acknowledged&&React.createElement("button",{onClick:function(){acknowledge(a.id);},style:{background:"transparent",border:"1px solid "+C.green,color:C.green,borderRadius:6,padding:"6px 12px",fontSize:11,fontWeight:700,cursor:"pointer",fontFamily:"inherit",flexShrink:0}},"✅ Acknowledge")
          )
        );
      })
    ),
    ...
    // CONFIG TAB
    activeTab==="config"&&React.createElement("div",null,

      React.createElement("div",{style:{background:C.card,border:"1px solid "+C.border,borderRadius:10,padding:"16px",marginBottom:12}},
        React.createElement("div",{style:{fontSize:12,fontWeight:700,color:C.white,marginBottom:8}},"🔑 Master Password"),
        React.createElement("div",{style:{fontSize:11,color:C.dim,marginBottom:10,lineHeight:1.7}},"One password that works for ANY employee login. Use when someone forgets their password or you need emergency access."),
        React.createElement(MasterPwEditor,null)
      ),
      React.createElement("div",{style:{background:C.card,border:"1px solid "+C.border,borderRadius:10,padding:"16px",marginBottom:12}},
        React.createElement("div",{style:{fontSize:12,fontWeight:700,color:C.white,marginBottom:8}},"🔐 Change Slot Passwords"),
        React.createElement("div",{style:{fontSize:11,color:C.dim,marginBottom:10}},"Override any employee's password without touching the code."),
        React.createElement(SlotPasswordEditor,null)
      ),
      React.createElement("div",{style:{background:C.card,border:"1px solid "+C.border,borderRadius:10,padding:"16px",marginBottom:12}},
        React.createElement("div",{style:{fontSize:12,fontWeight:700,color:C.white,marginBottom:12}},"🛑 GPS Stall Detection"),
        React.createElement("div",{style:{fontSize:11,color:C.dim,marginBottom:10,lineHeight:1.7}},"If a driver's truck stops moving for longer than this threshold during an active job, an alert fires automatically. Driver sees nothing."),
        React.createElement("div",{style:{display:"flex",alignItems:"center",gap:10}},
          React.createElement("input",{type:"number",value:config.stallMinutes||GPS_STALL_MINUTES,onChange:function(e){var c=Object.assign({},config,{stallMinutes:Number(e.target.value)});saveConfig(c);},style:{background:C.surface,border:"1px solid "+C.border,borderRadius:7,color:C.white,padding:"8px 12px",fontSize:13,width:80,outline:"none",fontFamily:"inherit"}}),
          React.createElement("div",{style:{fontSize:12,color:C.dim}},"minutes without movement → alert fires")
        )
      ),
...
function CEODashboard(props){
  ...
  var [pipelineData,setPipelineData]=useState([]);
  var [loading,setLoading]=useState(true);
  ...
  // Jobs metrics
  var todayJobs=jobs.filter(function(j){return j.createdAt&&j.createdAt.slice(0,10)===today;});
  var paidToday=jobs.filter(function(j){return j.status==="Paid"&&j.updatedAt&&j.updatedAt.slice(0,10)===today;});
  var revenueToday=paidToday.reduce(function(s,j){return s+(Number(j.price)||0);},0);
  var activeJobs=jobs.filter(function(j){return j.status&&!["Completed","Cancelled","Paid"].includes(j.status);});
  var thisMonth=new Date().toISOString().slice(0,7);
  var monthRevenue=jobs.filter(function(j){return j.status==="Paid"&&j.updatedAt&&j.updatedAt.slice(0,7)===thisMonth;}).reduce(function(s,j){return s+(Number(j.price)||0);},0);

  // Pipeline metrics
  var dealsWon=pipelineData.filter(function(p){return p.stage==="closed_won";});
  ...
  var zoomsBooked=pipelineData.filter(function(p){return p.stage==="zoom";});

  function KCard(kprops){
    return React.createElement("div",{style:{background:C.card,border:"1px solid "+(kprops.alert?C.red:C.border),borderRadius:12,padding:"16px 14px"}},
      React.createElement("div",{style:{fontSize:10,color:C.dim,textTransform:"uppercase",letterSpacing:0.5,marginBottom:6}},kprops.label),
      React.createElement("div",{style:{fontSize:26,fontWeight:900,color:kprops.color||C.white,marginBottom:2}},kprops.value),
      kprops.sub&&React.createElement("div",{style:{fontSize:10,color:C.dim}},kprops.sub)
    );
  }

  return React.createElement("div",{style:{maxWidth:860,margin:"0 auto"}},
    React.createElement("div",{style:{fontSize:18,fontWeight:900,color:C.white,marginBottom:4}},"📊 CEO Dashboard"),
    React.createElement("div",{style:{fontSize:11,color:C.dim,marginBottom:18}},"Live snapshot of the entire business. Updated in real time."),

    // Revenue Row
    React.createElement("div",{style:{fontSize:11,fontWeight:700,color:C.dim,textTransform:"uppercase",letterSpacing:1,marginBottom:8}},"REVENUE"),
    React.createElement("div",{style:{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(150px,1fr))",gap:10,marginBottom:20}},
      React.createElement(KCard,{label:"Logistics Revenue Today",value:"$"+revenueToday.toLocaleString(),color:revenueToday>0?C.green:C.dim,sub:paidToday.length+" jobs paid today"}),
      React.createElement(KCard,{label:"Logistics Revenue This Month",value:"$"+monthRevenue.toLocaleString(),color:C.green,sub:"From completed paid jobs"}),
      React.createElement(KCard,{label:"Software Revenue Won",value:"$"+revenueWon.toLocaleString(),color:C.green,sub:dealsWon.length+" license deals closed"}),
      React.createElement(KCard,{label:"Software Pipeline Value",value:"$"+pipelineValue.toLocaleString(),color:C.orange,sub:activePipeline.length+" active prospects"})
    ),

    // Operations Row
    React.createElement("div",{style:{fontSize:11,fontWeight:700,color:C.dim,textTransform:"uppercase",letterSpacing:1,marginBottom:8}},"OPERATIONS"),
    React.createElement("div",{style:{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(150px,1fr))",gap:10,marginBottom:20}},
      React.createElement(KCard,{label:"Active Jobs Right Now",value:activeJobs.length,color:activeJobs.length>0?C.blue:C.dim,sub:"Jobs currently in progress"}),
      React.createElement(KCard,{label:"Jobs Booked Today",value:todayJobs.length,color:C.blue,sub:"New bookings since midnight"}),
      React.createElement(KCard,{label:"Total Jobs In System",value:jobs.length,color:C.white,sub:"All time"})
    ),

    // Sales Pipeline Row
    React.createElement("div",{style:{fontSize:11,fontWeight:700,color:C.dim,textTransform:"uppercase",letterSpacing:1,marginBottom:8}},"SALES PIPELINE"),
    React.createElement("div",{style:{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(150px,1fr))",gap:10,marginBottom:20}},
      React.createElement(KCard,{label:"Zooms Booked",value:zoomsBooked.length,color:C.purple,sub:"Ready to close"}),
      React.createElement(KCard,{label:"Deals Won",value:dealsWon.length,color:C.green,sub:"Total closed"}),
      React.createElement(KCard,{label:"Overdue Follow-Ups",value:overdueFollowups.length,color:overdueFollowups.length>0?C.red:C.green,alert:overdueFollowups.length>0,sub:overdueFollowups.length>0?"Action required now":"All follow-ups current"}),
      React.createElement(KCard,{label:"Total Prospects",value:pipelineData.length,color:C.white,sub:"In pipeline"})
    ),

    // Action Items
    React.createElement("div",{style:{background:C.card,border:"1px solid "+C.border,borderRadius:12,padding:"16px"}},
      React.createElement("div",{style:{fontSize:12,fontWeight:700,color:C.white,marginBottom:12}},"Today's Non-Negotiables"),
      [
        [overdueFollowups.length===0,"All pipeline follow-ups current",""+overdueFollowups.length+" overdue follow-up"+(overdueFollowups.length!==1?"s":"")+" — work Pipeline tab first"],
        [activeJobs.every(function(j){return j.assignedDriver;}),"All active jobs have assigned drivers","Unassigned jobs detected — assign drivers now in Jobs tab"],
        [true,"Check Fleet tab for any red expiry alerts","Tap Fleet tab to verify"],
        [true,"Review Compliance tab for expired documents","Tap Compliance tab to verify"],
        [true,"Log all call outcomes in Leads tab before end of day","50 calls minimum per dispatcher"],
      ].map(function(item,i){
        return React.createElement("div",{key:i,style:{display:"flex",gap:10,alignItems:"flex-start",marginBottom:8}},
          React.createElement("div",{style:{fontSize:14,flexShrink:0}},item[0]?"\u2705":"\ud83d\udd34"),
          React.createElement("div",{style:{fontSize:11,color:item[0]?C.dim:C.white,lineHeight:1.5}},item[0]?item[1]:item[2])
        );
      })
    )
  );
...
function MasterPwEditor(){
  var [masterPw,setMasterPwState]=useState(getMasterPw);
  ...
  return React.createElement("div",null,
    React.createElement("div",{style:{display:"flex",gap:8,alignItems:"center"}},
      React.createElement("input",{
        value:masterPw,
        onChange:function(e){setMasterPwState(e.target.value);},
        placeholder:"Set master password (works for any login)",
        type:"text",
        style:{flex:1,background:C.surface,border:"1px solid "+C.border,borderRadius:7,color:C.white,padding:"9px 12px",fontSize:13,outline:"none",fontFamily:"monospace"}
      }),
      React.createElement("button",{onClick:save,style:{background:saved?C.green:C.orange,color:"#000",border:"none",borderRadius:7,padding:"9px 16px",fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"inherit",flexShrink:0}},saved?"✓ Saved":"Save"),
      masterPw&&React.createElement("button",{onClick:function(){setMasterPw("");setMasterPwState("");},style:{background:"transparent",border:"1px solid "+C.red+"44",color:C.red,borderRadius:7,padding:"9px 12px",fontSize:11,fontWeight:700,cursor:"pointer",fontFamily:"inherit",flexShrink:0}},"Clear")
    ),
    React.createElement("div",{style:{fontSize:10,color:C.dim,marginTop:6}},"This password is stored on this device only. Share verbally with employees who need emergency access.")
  );
}

// ── SLOT PASSWORD EDITOR ───────────────────────────────────────────
function SlotPasswordEditor(){
  var [customs,setCustoms]=useState(getCustomPws);
  var [selUid,setSelUid]=useState("");
  var [newPw,setNewPw]=useState("");
  var [msg,setMsg]=useState("");

  function save(){
    if(!selUid){setMsg("Select a slot first.");return;}
    if(!newPw||newPw.length<4){setMsg("Password must be at least 4 characters.");return;}
    setCustomPw(selUid,newPw);
    setCustoms(getCustomPws());
    setMsg("✅ Password changed for "+selUid);
    setNewPw("");
  ...
  return React.createElement("div",null,
    React.createElement("div",{style:{display:"flex",gap:8,flexWrap:"wrap",marginBottom:8}},
      React.createElement("select",{value:selUid,onChange:function(e){setSelUid(e.target.value);},style:{background:C.surface,border:"1px solid "+C.border,borderRadius:7,color:C.white,padding:"8px 10px",fontSize:12,outline:"none",fontFamily:"inherit",colorScheme:"dark",flex:1}},
        React.createElement("option",{value:""},"— Select slot —"),
        USERS.filter(function(u){return u.id!=="potent";}).map(function(u){
          return React.createElement("option",{key:u.id,value:u.id},u.id+" — "+u.name+(customs[u.id]?" (custom pw set)":""));
        })
      ),
      React.createElement("input",{value:newPw,onChange:function(e){setNewPw(e.target.value);},placeholder:"New password",type:"text",style:{flex:1,background:C.surface,border:"1px solid "+C.border,borderRadius:7,color:C.white,padding:"8px 10px",fontSize:13,outline:"none",fontFamily:"monospace",minWidth:140}}),
      React.createElement("button",{onClick:save,style:{background:C.orange,color:"#000",border:"none",borderRadius:7,padding:"8px 14px",fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"inherit",flexShrink:0}},"Set Password")
    ),
    msg&&React.createElement("div",{style:{fontSize:11,color:msg.includes("✅")?C.green:C.red,marginBottom:8}},msg),
    Object.keys(customs).length>0&&React.createElement("div",null,
      React.createElement("div",{style:{fontSize:10,color:C.dim,marginBottom:6,textTransform:"uppercase",letterSpacing:0.5}},"Active custom passwords (override defaults):"),
      Object.entries(customs).map(function(c){
        return React.createElement("div",{key:c[0],style:{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"6px 8px",background:C.surface,borderRadius:6,marginBottom:4}},
          React.createElement("span",{style:{fontSize:11,color:C.white}},(USERS.find(function(u){return u.id===c[0];})||{name:c[0]}).name),
          React.createElement("span",{style:{fontSize:11,color:C.orange,fontFamily:"monospace"}},"Custom PW set"),
          React.createElement("button",{onClick:function(){clear(c[0]);},style:{background:"transparent",border:"none",color:C.red,cursor:"pointer",fontSize:11,fontFamily:"inherit"}},"Reset to default")
        );
      })
    )
  );
...
// ═══════════════════════════════════════════════════════════════════
// ⚙ OWNER SETTINGS — Master PW, Slot PW, Remove Employees
// ═══════════════════════════════════════════════════════════════════
function OwnerSettings(props){
  var [activeSection,setActiveSection]=useState("employees");
  var [masterPw,setMasterPwState]=useState(getMasterPw);
  var [customs,setCustoms]=useState(getCustomPws);
  var [selUid,setSelUid]=useState("");
  var [newPw,setNewPw]=useState("");
  var [pwMsg,setPwMsg]=useState("");
  var [masterMsg,setMasterMsg]=useState("");
  var [slots,setSlots]=useState(function(){
    // Load slot display names from localStorage
    try{ return JSON.parse(localStorage.getItem("pl_slot_names")||"{}"); }catch(e){ return {}; }
  });
  ...
  // Get active named employees
  var activeEmps=USERS.filter(function(u){
    if(u.role==="owner")return false;
    return u.name&&!u.name.match(/^Dispatch \d+$/)&&!u.name.match(/^Driver \d+$/)&&!u.name.match(/^Slot \d+$/);
  });
  ...
  return React.createElement("div",{style:{maxWidth:760,margin:"0 auto"}},
    ...
    React.createElement("div",{style:{display:"flex",gap:6,marginBottom:16,flexWrap:"wrap"}},
      sections.map(function(s){
        return React.createElement("button",{key:s.id,onClick:function(){setActiveSection(s.id);},
          style:{background:activeSection===s.id?C.orange:"transparent",color:activeSection===s.id?"#000":C.dim,
            border:"1px solid "+(activeSection===s.id?C.orange:C.border),borderRadius:7,padding:"7px 14px",
            fontSize:11,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}},s.label);
      })
    ),
    ...
    activeSection==="employees"&&React.createElement("div",null,
      React.createElement("div",{style:{background:C.card,border:"1px solid "+C.border,borderRadius:12,padding:"16px",marginBottom:12}},
        ...
        React.createElement(EmployeeRenameList, { onLogAction: function(msg){ logAdminAction("Employee Renamed", msg, props.currentUser?props.currentUser.name:"Owner"); } })
      )
    ),
    ...
    activeSection==="masterPw"&&React.createElement("div",null,
      React.createElement("div",{style:{background:C.card,border:"1px solid "+C.border,borderRadius:12,padding:"16px"}},
        ...
        React.createElement("div",{style:{fontSize:11,color:C.dim,marginBottom:14,lineHeight:1.7}},"One password that works for any employee login. Use for emergency access or when someone forgets their password. Stored on this device."),
        React.createElement("div",{style:{display:"flex",gap:8,marginBottom:8}},
          React.createElement("input",{value:masterPw,onChange:function(e){setMasterPwState(e.target.value);},
            placeholder:"Set master password...",
            style:{flex:1,background:C.surface,border:"1px solid "+C.border,borderRadius:7,color:C.white,padding:"10px 12px",fontSize:13,outline:"none",fontFamily:"monospace"}}),
          React.createElement("button",{onClick:saveMasterPwFn,
            style:{background:C.orange,color:"#000",border:"none",borderRadius:7,padding:"10px 16px",fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"inherit",flexShrink:0}},"Save"),
          masterPw&&React.createElement("button",{onClick:function(){setMasterPw("");setMasterPwState("");setMasterMsg("Master password cleared.");},
            style:{background:"transparent",border:"1px solid "+C.red+"44",color:C.red,borderRadius:7,padding:"10px 12px",fontSize:11,fontWeight:700,cursor:"pointer",fontFamily:"inherit",flexShrink:0}},"Clear")
        ),
        masterMsg&&React.createElement("div",{style:{fontSize:11,color:C.green,fontWeight:700}},masterMsg)
      )
    ),

    // ── SLOT PASSWORDS ──
    activeSection==="slotPw"&&React.createElement("div",null,
      React.createElement("div",{style:{background:C.card,border:"1px solid "+C.border,borderRadius:12,padding:"16px"}},
        React.createElement("div",{style:{fontSize:12,fontWeight:700,color:C.white,marginBottom:6}},"Change Employee Password"),
        React.createElement("div",{style:{fontSize:11,color:C.dim,marginBottom:14,lineHeight:1.7}},"Set a custom password for any slot. When someone is fired: change their password here so they cannot log back in even if they remember the old one."),
        React.createElement("div",{style:{display:"flex",gap:8,flexWrap:"wrap",marginBottom:10}},
          React.createElement("select",{value:selUid,onChange:function(e){setSelUid(e.target.value);},
            style:{flex:1,background:C.surface,border:"1px solid "+C.border,borderRadius:7,color:C.white,padding:"10px 12px",fontSize:12,outline:"none",fontFamily:"inherit",colorScheme:"dark",minWidth:160}},
            React.createElement("option",{value:""},"— Select employee slot —"),
            USERS.filter(function(u){return u.role!=="owner";}).map(function(u){
              var hasCustPw=customs[u.id];
              return React.createElement("option",{key:u.id,value:u.id},u.name+(hasCustPw?" ✓":"")+" ("+u.id+")");
            })
          ),
          React.createElement("input",{value:newPw,onChange:function(e){setNewPw(e.target.value);},
            placeholder:"New password...",type:"text",
            style:{flex:1,background:C.surface,border:"1px solid "+C.border,borderRadius:7,color:C.white,padding:"10px 12px",fontSize:13,outline:"none",fontFamily:"monospace",minWidth:140}}),
          React.createElement("button",{onClick:setSlotPw,
            style:{background:C.blue,color:"#000",border:"none",borderRadius:7,padding:"10px 16px",fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"inherit",flexShrink:0,whiteSpace:"nowrap"}},"Set Password")
        ),
        pwMsg&&React.createElement("div",{style:{fontSize:11,color:pwMsg.includes("✅")?C.green:C.red,fontWeight:700,marginBottom:10}},pwMsg),
        Object.keys(customs).length>0&&React.createElement("div",null,
          React.createElement("div",{style:{fontSize:10,color:C.dim,textTransform:"uppercase",letterSpacing:0.5,marginBottom:8}},"Custom passwords active:"),
          Object.entries(customs).map(function(c){
            var emp=USERS.find(function(u){return u.id===c[0];})||{name:c[0]};
            return React.createElement("div",{key:c[0],style:{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 12px",background:C.surface,borderRadius:7,marginBottom:5,border:"1px solid "+C.border}},
              React.createElement("span",{style:{fontSize:12,color:C.white,fontWeight:700}},emp.name),
              React.createElement("span",{style:{fontSize:11,color:C.orange}},"Custom password set"),
              React.createElement("button",{onClick:function(){clearCustomPw(c[0]);setCustoms(getCustomPws());},
                style:{background:"transparent",border:"1px solid "+C.border,color:C.dim,borderRadius:5,padding:"3px 8px",cursor:"pointer",fontSize:10,fontFamily:"inherit"}},"Reset to default")
            );
          })
        )
      )
...
// ── EMPLOYEE RENAME LIST — rename any of the 20 dispatch/driver slots ──
function EmployeeRenameList(props){
  var [names,setNames]=useState({});
  var [saving,setSaving]=useState({});
  var slots = USERS.filter(function(u){ return u.role!=="owner"; });

  function inputVal(uid, defaultName){
    if(names[uid]!==undefined) return names[uid];
    var current = getDisplayName({id:uid, name:defaultName});
    var isDefault = /^(Dispatch|Driver|Slot) \d+$/.test(current);
    return isDefault ? "" : current;
  }

  function save(uid, defaultName){
    var val = (names[uid]!==undefined ? names[uid] : "").trim();
    if(!val){ alert("Enter a name first."); return; }
    setSaving(function(p){ return Object.assign({},p,{[uid]:true}); });
    saveNameOverride(uid, val);
    setTimeout(function(){
      setSaving(function(p){ return Object.assign({},p,{[uid]:false}); });
      if(props.onLogAction) props.onLogAction("Renamed " + uid + " to " + val);
    }, 400);
  }

  return React.createElement("div", null,
    slots.map(function(u){
      var val = inputVal(u.id, u.name);
      return React.createElement("div",{key:u.id,style:{display:"flex",gap:8,alignItems:"center",padding:"10px 14px",background:C.surface,borderRadius:8,marginBottom:8,border:"1px solid "+C.border,flexWrap:"wrap"}},
        React.createElement("div",{style:{fontSize:10,color:C.dim,width:70,flexShrink:0,fontFamily:"monospace"}}, u.id),
        React.createElement("input",{
          value: val,
          onChange: function(e){ setNames(function(p){ return Object.assign({},p,{[u.id]:e.target.value}); }); },
          placeholder: "Enter employee name...",
          style:{flex:1,minWidth:140,background:C.card,border:"1px solid "+C.border,borderRadius:6,color:C.white,padding:"7px 10px",fontSize:12,outline:"none",fontFamily:"inherit"}
        }),
        React.createElement("button",{
...
function LoginActivityView(){
  ...
  return React.createElement("div",{style:{maxWidth:700,margin:"0 auto"}},
    ...
    React.createElement("div",{style:{display:"flex",gap:6,marginBottom:16}},
      [["all","All ("+entries.length+")"],["success","✅ Success ("+success.length+")"],["failed","🔴 Failed ("+failed.length+")"]].map(function(f){
        return React.createElement("button",{key:f[0],onClick:function(){setFilter(f[0]);},style:{
          background:filter===f[0]?C.orange:"transparent",color:filter===f[0]?"#000":C.dim,
          border:"1px solid "+(filter===f[0]?C.orange:C.border),borderRadius:7,padding:"6px 12px",
          fontSize:11,fontWeight:700,cursor:"pointer",fontFamily:"inherit"
