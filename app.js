...
function PayrollDashboard(props) {
    ...
    var repEarnings = {};
    var dispatchUsers = users.filter(function (u) { return u.id !== "potent"; });
    dispatchUsers.forEach(function (u) {
        ...
        repEarnings[u.id] = { user: u, logRate, osRate, lbRate, jobs: myJobs.length, revenue, logRev, osRev, lbRev, commission, alreadyPaid, outstanding };
    });
                ...
                React.createElement("div", { style: { display: "grid", gridTemplateColumns: "1fr auto", gap: 12, alignItems: "start" } },
                    React.createElement("div", null,
                        React.createElement("div", { style: { fontSize: 14, fontWeight: 800, color: C.white, marginBottom: 4 } },
                            ...
                            " ",
                            rep.user.name),
                        React.createElement("div", { style: { display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(90px,1fr))", gap: 8, marginBottom: 10 } }, [["Jobs", rep.jobs, C.white], ["Logistics", "$" + (rep.logRev || 0).toLocaleString(), C.blue], ["POTENT OS", "$" + (rep.osRev || 0).toLocaleString(), "#9F7AEA"], ["Loadboard", "$" + (rep.lbRev || 0).toLocaleString(), C.green], ["Total Rev", "$" + rep.revenue.toLocaleString(), C.orange], ["Commission", "$" + rep.commission.toLocaleString(), C.green], ["Paid", "$" + rep.alreadyPaid.toLocaleString(), C.dim], ["Outstanding", "$" + rep.outstanding.toLocaleString(), rep.outstanding > 0 ? C.red : C.green]].map(function (s) {
